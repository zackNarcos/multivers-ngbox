...
exports.getUnUsedProjectComponents = exports.getAllComponents = void 0;
const node_console_1 = require("node:console");
const file_reader_1 = require("../../common/file-reader");
const path = require("path");
const getAllComponents = (projectPath) => {
    const componentFiles = (0, file_reader_1.getElementFiles)(projectPath, 'component');
    let components = [];
    componentFiles.forEach(componentFile => {
        const componentContent = (0, file_reader_1.getFileContent)(projectPath, componentFile);
...
const getUnUsedProjectComponents = (projectPath, type) => {
    const components = (0, exports.getAllComponents)(projectPath);
    //log(components);
    //return components;
    ...
    components.forEach((component) => {
        ...
        try {
            const componentContent = (0, file_reader_1.getFileContent)(projectPath, absolutePath);
        ...
        const componentTsContent = (0, file_reader_1.getFileContent)(projectPath, component.componentTsPath);
        const classMatch = componentTsContent.match(new RegExp(`\\b${component.className}\\b`));
        if (classMatch) {
            usedClasses.add(component.className);
        }
    });
    (0, node_console_1.log)(nb);
    if (type === "used") {
        ...
        (0, node_console_1.log)("in ", components.length, " components, ", a.length, " are used", unUsed);
        return a;
    }
    else {
        const a = components.filter((component) => !usedSelectors.has(component.selector) && !usedClasses.has(component.className));
        (0, node_console_1.log)("in ", components.length, " components, ", a.length, " are not used");
        return a;
    }
};
exports.getUnUsedProjectComponents = getUnUsedProjectComponents;
